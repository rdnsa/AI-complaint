      ...
      {PILIHAN.map((p) => (
        <button
          ...
          className={`rounded-full font-bold transition ${kecil ? 'px-2 py-0.5 text-[10px]' : 'px-2.5 py-1 text-xs'} ${
            bahasa === p.kode ? 'bg-white text-tetap-maroon' : 'text-white/80 hover:text-white'
          }`}
        >
          {p.label}
        </button>
...
function TombolTema({ kecil = false }: { kecil?: boolean }) {
  const { gelap, ubah } = useTema();
  const { t } = useBahasa();
  const tinggi = kecil ? 'h-6 w-11' : 'h-8 w-14';
  const knop = kecil ? 'h-5 w-5 text-[11px]' : 'h-7 w-7 text-sm';
  const geser = kecil ? 'translate-x-5' : 'translate-x-6';
  return (
    <button
      type="button"
      role="switch"
      aria-checked={gelap}
      onClick={() => ubah(!gelap)}
      aria-label={gelap ? t('tema.ke_terang') : t('tema.ke_gelap')}
      title={gelap ? t('tema.ke_terang') : t('tema.ke_gelap')}
      className={`relative shrink-0 rounded-full p-0.5 ring-1 ring-white/25 transition ${tinggi} ${
        gelap ? 'bg-white/30' : 'bg-white/15'
      }`}
    >
      <span
        aria-hidden
        className={`grid place-items-center rounded-full bg-white text-tetap-maroon shadow transition-transform duration-200 ${knop} ${
          gelap ? geser : 'translate-x-0'
        }`}
      >
        {gelap ? '🌙' : '☀️'}
      </span>
        ...
        <div className={`mx-auto max-w-5xl px-4 ${ramping ? 'py-4' : 'py-7'}`}>
          <div className="flex items-center justify-between gap-3">
            <Link to="/" className="flex items-center gap-2.5">
              ...
              <span className="text-[10px] font-semibold uppercase leading-tight tracking-[0.1em] text-tetap-krem sm:text-[11px]">
                {t('kop.universitas')}
                <br />
          ...
          {keterangan && (
            <p className="mt-1.5 max-w-xl text-sm leading-relaxed text-tetap-krem">{keterangan}</p>
          )}
        </div>
